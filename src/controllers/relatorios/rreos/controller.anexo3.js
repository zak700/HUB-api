...
const permReceitas = {
  ...
  // -- é necessário colocar os valores certos no futuro

  contribuicaoServidor: ["11125001", "11125003", "11125004"],
  conpensacaoFinanceira: ["11125001", "11125002", "11125003"],
  rendimentoRecPrevidenciario: ["11125002", "11125003", "11125004"],
  deducaoFUNDEB: ["11125001", "11125002", "11125004"],

  transObgIndividual: ["11125001", "11125002", "11125003", "11125004"], // (IV)
  transObgBancada: ["11125301"], // (VI)
  transObgRemAgentes: ["11130341"], // (VII)
  outrasDeducoes: ["11130341"], //  (VIII)
};
...
async function getRreoAnexo(req, res) {
  try {
    ...
    const getInfo = (perm, deducao) => {
      const filtered = dataHelper.filterOrg(rec.filter((e) => {
        const yearData = dataHelper.dataToYear(e.data);
        const monthData = parseInt(e.data.substring(0, 2));
        if (deducao === true) {
          return (
            permReceitas[perm].includes(e.content.rubrica.substring(1)) &&
            e.content.rubrica.substring(0, 1) === "9"
          );
        }
    ...
    output.response.push(
      ...
      dataHelper.sumInfo(
        [
          getInfo("contribuicaoServidor", true),
          getInfo("conpensacaoFinanceira", true),
          getInfo("rendimentoRecPrevidenciario", true),
          getInfo("deducaoFUNDEB", true),
        ],
        14
      ), // deduções
      getInfo("contribuicaoServidor", true),
      ...
      dataHelper.sumInfo(
        [
          ...
          getInfo("transObgIndividual"),
        ],
        14,
        "sub"
      ), //                             (V)
      getInfo("transObgBancada"), //    (VI)
      getInfo("transObgRemAgentes"), // (VII)
      getInfo("outrasDeducoes"), //     (VIII)
      dataHelper.sumInfo(
        [
          dataHelper.sumInfo(
            [
